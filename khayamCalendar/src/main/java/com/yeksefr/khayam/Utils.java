...
import com.yeksefr.khayam.CalendarUtils.TimeZoneUtils;
...
public class Utils {
    ...
    public static final int MONDAY_BEFORE_JULIAN_EPOCH = JalaliTime.EPOCH_JULIAN_DAY - 3;
    ...
    public static final int YEAR_MIN = 1364;// 1970;
    public static final int YEAR_MAX = 1407;// 2036;
    ...
    // was created.
    static final String SHARED_PREFS_NAME = "com.yeksefr.khayam_preferences";
    ...
    **/
     ...
     */
    public static String formatMonthYear(Context context, JalaliTime time) {
//        int flags = JalaliDateUtils.FORMAT_SHOW_DATE | JalaliDateUtils.FORMAT_NO_MONTH_DAY
//                | JalaliDateUtils.FORMAT_SHOW_YEAR;
        long millis = time.toMillis(true);
        return JalaliDateUtils.getMonthAndYearString(context, millis, JalaliDateUtils.LENGTH_SHORT);
    }
    ...
    public static int getWeeksSinceEpochFromJulianDay(int julianDay, int firstDayOfWeek) {
        int diff = JalaliTime.THURSDAY - firstDayOfWeek;
        if (diff < 0) {
            diff += 7;
        }
        int refDay = JalaliTime.EPOCH_JULIAN_DAY - diff;
    ...
    public static int getFirstDayOfWeek(Context context) {
        ...
        if (GeneralPreferences.WEEK_START_DEFAULT.equals(pref)) {
            startDay = Calendar.SATURDAY;//new JalaliCalendar().getFirstDayOfWeek();
        } else {
            startDay = Integer.parseInt(pref);
        }

        if (startDay == Calendar.SATURDAY) {
            return JalaliTime.SATURDAY;
        } else if (startDay == Calendar.MONDAY) {
            return JalaliTime.MONDAY;
        } else {
            return JalaliTime.SUNDAY;
        }
        */
    ...
    public static int convertDayOfWeekFromTimeToCalendar(int timeDayOfWeek) {
        switch (timeDayOfWeek) {
            case JalaliTime.MONDAY:
                return Calendar.MONDAY;
            case JalaliTime.TUESDAY:
                return Calendar.TUESDAY;
            case JalaliTime.WEDNESDAY:
                return Calendar.WEDNESDAY;
            case JalaliTime.THURSDAY:
                return Calendar.THURSDAY;
            case JalaliTime.FRIDAY:
                return Calendar.FRIDAY;
            case JalaliTime.SATURDAY:
                return Calendar.SATURDAY;
            case JalaliTime.SUNDAY:
    ...
    public static boolean isSaturday(int column, int firstDayOfWeek) {
        return (firstDayOfWeek == JalaliTime.SUNDAY && column == 6)
                || (firstDayOfWeek == JalaliTime.MONDAY && column == 5)
                || (firstDayOfWeek == JalaliTime.SATURDAY && column == 0);
    }
    ...
    public static boolean isSunday(int column, int firstDayOfWeek) {
        return (firstDayOfWeek == JalaliTime.SUNDAY && column == 0)
                || (firstDayOfWeek == JalaliTime.MONDAY && column == 6)
                || (firstDayOfWeek == JalaliTime.SATURDAY && column == 1);
    }
     ...
     */
    public static long convertAlldayUtcToLocal(JalaliTime recycle, long utcTime, String tz) {
        if (recycle == null) {
            recycle = new JalaliTime();
        }
        recycle.timezone = JalaliTime.TIMEZONE_UTC;
        recycle.set(utcTime);
        recycle.timezone = tz;
        return recycle.normalize(true);
    }

    public static long convertAlldayLocalToUTC(JalaliTime recycle, long localTime, String tz) {
        if (recycle == null) {
            recycle = new JalaliTime();
        }
        recycle.timezone = tz;
     ...
     * @param tz The time zone to convert this time to.
     */
    public static long getNextMidnight(JalaliTime recycle, long theTime, String tz) {
        if (recycle == null) {
            recycle = new JalaliTime();
        }
        recycle.timezone = tz;
     ...
     * @return
     */
    public static int getJalaliWeekNumberFromTime(long millisSinceEpoch, Context context) {
    	JalaliTime weekTime = new JalaliTime(getTimeZone(context, null));
        ...
        // week
//        if (weekTime.weekDay == JalaliTime.SUNDAY
//                && (firstDayOfWeek == JalaliTime.SUNDAY || firstDayOfWeek == JalaliTime.SATURDAY)) {
//            weekTime.monthDay++;
//            weekTime.normalize(true);
//        } else if (weekTime.weekDay == JalaliTime.SATURDAY && firstDayOfWeek == JalaliTime.SATURDAY) {
//            weekTime.monthDay += 2;
//            weekTime.normalize(true);
//        }
        return weekTime.getJalaliWeekNumber();
    }
    ...
    public static void setMidnightUpdater(Handler h, Runnable r, String timezone) {
        ...
        JalaliTime time = new JalaliTime(timezone);
        time.set(now);
    ...
    public static String getDisplayedDatetime(long startMillis, long endMillis, long currentMillis,
        ...
        int flagsDate = JalaliDateUtils.FORMAT_SHOW_DATE | JalaliDateUtils.FORMAT_SHOW_WEEKDAY;
        int flagsTime = JalaliDateUtils.FORMAT_SHOW_TIME;
        if (DateFormat.is24HourFormat(context)) {
            flagsTime |= JalaliDateUtils.FORMAT_24HOUR;
        }

        JalaliTime currentTime = new JalaliTime(localTimezone);
...
//                datetimeString = JalaliDateUtils.formatDateRange(context, f, startMillis,
//                        endMillis, flagsDate, Time.TIMEZONE_UTC).toString();
            ...
            if (singleDayEvent(startMillis, endMillis, currentTime.gmtoff)) {
                ...
                if (TODAY == todayOrTomorrow) {
                    ...
                    String dateString = Utils.formatDateRange(context, startMillis, endMillis,
                            flagsDate);
                    dateString = JalaliDateUtils.formatDateRange(context, startMillis, endMillis, flagsDate);
                ...
                int flagsDatetime = flagsDate | flagsTime | JalaliDateUtils.FORMAT_ABBREV_MONTH |
    ...
    public static String getDisplayedTimezone(long startMillis, String localTimezone,
        ...
        if (!TextUtils.equals(localTimezone, eventTimezone)) {
            ...
            if (tz == null || tz.getID().equals("GMT")) {
            	...
            	JalaliTime startTime = new JalaliTime(localTimezone);
    ...
    private static boolean singleDayEvent(long startMillis, long endMillis, long localGmtOffset) {
        ...
        // time end-1.
        int startDay = JalaliTime.getJulianDay(startMillis, localGmtOffset);
        int endDay = JalaliTime.getJulianDay(endMillis - 1, localGmtOffset);
    ...
    private static int isTodayOrTomorrow(Resources r, long dayMillis,
        ...
        int startDay = JalaliTime.getJulianDay(dayMillis, localGmtOffset);
        int currentDay = JalaliTime.getJulianDay(currentMillis, localGmtOffset);
    ...
    public static void setTodayIcon(LayerDrawable icon, Context c, String timezone) {
        ...
        JalaliTime now =  new JalaliTime(timezone);
        now.setToNow();
        now.normalize(false);
        
        today.setDayOfMonth(now.jalaliMonthDay);
        icon.mutate();
     ...
     * links. If lastDitchGeo is true, then if no links are found in the CustomTextView, the entire
