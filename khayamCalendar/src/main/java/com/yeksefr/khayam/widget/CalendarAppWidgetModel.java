...
class CalendarAppWidgetModel {
    ...
    static class EventInfo {
        int visibWhen; // Visibility value for When CustomTextView (View.GONE or View.VISIBLE)
        String when;
        int visibWhere; // Visibility value for Where CustomTextView (View.GONE or View.VISIBLE)
        String where;
        int visibTitle; // Visibility value for Title CustomTextView (View.GONE or View.VISIBLE)
        String title;
    ...
    public CalendarAppWidgetModel(Context context, String timeZone) {
        ...
        mTodayJulianDay = JalaliTime.getJulianDay(mNow, time.gmtoff);
    ...
    public void buildFromCursor(Cursor cursor, String timeZone) {
        final JalaliTime recycle = new JalaliTime(timeZone);
        ...
        recycle.setToNow();
        mShowTZ = !TextUtils.equals(timeZone, JalaliTime.getCurrentTimezone());
        if (mShowTZ) {
    ...
    private EventInfo populateEventInfo(long eventId, boolean allDay, long start, long end,
        ...
        int visibWhen;
        int flags = JalaliDateUtils.FORMAT_ABBREV_ALL;
        visibWhen = View.VISIBLE;
        if (allDay) {
            flags |= JalaliDateUtils.FORMAT_SHOW_DATE;
            whenString.append(JalaliDateUtils.formatDateRange(mContext, start, end, flags));
        } else {
    ...
    private DayInfo populateDayInfo(int julianDay, JalaliTime recycle) {
        long millis = recycle.setJulianDay(julianDay);
        int flags = JalaliDateUtils.FORMAT_ABBREV_ALL | JalaliDateUtils.FORMAT_SHOW_DATE;

        String label;
        if (julianDay == mTodayJulianDay + 1) {
            label = mContext.getString(R.string.agenda_tomorrow,
            		JalaliDateUtils.formatDateRange(mContext, millis, millis, flags).toString());
        } else {
            flags |= JalaliDateUtils.FORMAT_SHOW_WEEKDAY;
            label = JalaliDateUtils.formatDateRange(mContext, millis, millis, flags);
        }
