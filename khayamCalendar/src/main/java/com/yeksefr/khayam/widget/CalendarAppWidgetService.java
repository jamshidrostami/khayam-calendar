...
import com.yeksefr.khayam.widget.CalendarAppWidgetModel.DayInfo;
import com.yeksefr.khayam.widget.CalendarAppWidgetModel.EventInfo;
import com.yeksefr.khayam.widget.CalendarAppWidgetModel.RowInfo;
...
public class CalendarAppWidgetService extends RemoteViewsService {
    ...
    private static final long SEARCH_DURATION = MAX_DAYS * JalaliDateUtils.DAY_IN_MILLIS;

    /**
     * Update interval used when no next-update calculated, or bad trigger time in past.
     * Unit: milliseconds.
     */
    private static final long UPDATE_TIME_NO_EVENTS = JalaliDateUtils.HOUR_IN_MILLIS * 6;
    ...
    public static class CalendarFactory extends BroadcastReceiver implements
        ...
        public RemoteViews getViewAt(int position) {
            ...
            if (rowInfo.mType == RowInfo.TYPE_DAY) {
                ...
                updateCustomTextView(views, R.id.date, View.VISIBLE, dayInfo.mDayLabel);
                return views;
            } else {
                RemoteViews views;
                ...
                if (!eventInfo.allDay) {
                    updateCustomTextView(views, R.id.when, eventInfo.visibWhen, eventInfo.when);
                    updateCustomTextView(views, R.id.where, eventInfo.visibWhere, eventInfo.where);
                }
                updateCustomTextView(views, R.id.title, eventInfo.visibTitle, eventInfo.title);
                ...
                if (eventInfo.allDay) {
                    ...
                    JalaliTime recycle = new JalaliTime();
        ...
        private Uri createLoaderUri() {
            ...
            long begin = now - JalaliDateUtils.DAY_IN_MILLIS;
            long end = now + SEARCH_DURATION + JalaliDateUtils.DAY_IN_MILLIS;
        ...
        static void updateCustomTextView(RemoteViews views, int id, int visibility, String string) {
        ...
        public void onLoadComplete(Loader<Cursor> loader, Cursor cursor) {
            ...
            synchronized (mLock) {
                ...
                JalaliTime time = new JalaliTime(Utils.getTimeZone(mContext, null));
                time.setToNow();

                if (time.normalize(true) != sLastUpdateTime) {
                	JalaliTime time2 = new JalaliTime(Utils.getTimeZone(mContext, null));
    ...
    static String formatDebugTime(long unixTime, long now) {
        ...
        if (delta > JalaliDateUtils.MINUTE_IN_MILLIS) {
            delta /= JalaliDateUtils.MINUTE_IN_MILLIS;
            return String.format("[%d] %s (%+d mins)", unixTime,
                    time.format("%H:%M:%S"), delta);
        } else {
            delta /= JalaliDateUtils.SECOND_IN_MILLIS;
