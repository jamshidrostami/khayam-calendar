...
import com.yeksefr.khayam.ContactsAsyncHelper;
...
import com.yeksefr.khayam.event.EditEventHelper.AttendeeItem;
...
public class AttendeesView extends LinearLayout implements View.OnClickListener {
    ...
    // CustomTextView shown at the top of each type of attendees
    // e.g.
    // Yes  <-- divider
    ...
    private View constructDividerView(CharSequence label) {
        final CustomTextView CustomTextView =
            (CustomTextView)mInflater.inflate(R.layout.event_info_label, this, false);
    ...
    private void updateDividerViewLabel(View divider, CharSequence label, int count) {
        if (count <= 0) {
            ((CustomTextView)divider).setText(label);
        }
        else {
            ((CustomTextView)divider).setText(label + " (" + count + ")");
        }
    }
    ...
    private View updateAttendeeView(AttendeeItem item) {
        ...
        final CustomTextView nameView = (CustomTextView) view.findViewById(R.id.name);
    ...
    public boolean contains(Attendee attendee) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (view instanceof CustomTextView) { // divider
                continue;
            }
