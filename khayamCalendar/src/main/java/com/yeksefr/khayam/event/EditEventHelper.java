...
import com.yeksefr.khayam.AbstractCalendarActivity;
import com.yeksefr.khayam.AsyncQueryService;
...
public class EditEventHelper {
    ...
    protected long constructDefaultStartTime(long now) {
    	JalaliTime defaultStart = new JalaliTime();
        ...
        if (now < defaultStartMillis) {
            ...
            return defaultStartMillis + 30 * JalaliDateUtils.MINUTE_IN_MILLIS;
        }
    }
    ...
    protected long constructDefaultEndTime(long startTime) {
        return startTime + JalaliDateUtils.HOUR_IN_MILLIS;
    }
    ...
    void checkTimeDependentFields(CalendarEventModel originalModel, CalendarEventModel model,
        ...
        if (modifyWhich == MODIFY_ALL) {
            ...
            if (newAllDay) {
            	JalaliTime time = new JalaliTime(JalaliTime.TIMEZONE_UTC);
    ...
    public String updatePastEvents(ArrayList<ContentProviderOperation> ops,
        ...
        JalaliTime dtstart = new JalaliTime();
        ...
        if (origRecurrence.count > 0) {
        	...
        	JalaliTime untilTime = new JalaliTime();
            untilTime.timezone = JalaliTime.TIMEZONE_UTC;
            ...
            if (origAllDay) {
                ...
                dtstart.second = 0;
                dtstart.allDay = true;
                dtstart.timezone = JalaliTime.TIMEZONE_UTC;
            }
    ...
    void addRecurrenceRule(ContentValues values, CalendarEventModel model) {
        ...
        if (end >= start) {
            if (isAllDay) {
                ...
                long days = (end - start + JalaliDateUtils.DAY_IN_MILLIS - 1)
                        / JalaliDateUtils.DAY_IN_MILLIS;
                duration = "P" + days + "D";
            } else {
                // otherwise compute the duration in seconds
                long seconds = (end - start) / JalaliDateUtils.SECOND_IN_MILLIS;
    ...
    static void updateRecurrenceRule(int selection, CalendarEventModel model,
        ...
        if (selection == DOES_NOT_REPEAT) {
            ...
            JalaliTime startTime = new JalaliTime(model.mTimezone);
    ...
    ContentValues getContentValuesFromModel(CalendarEventModel model) {
        ...
        JalaliTime startTime = new JalaliTime(timezone);
        JalaliTime endTime = new JalaliTime(timezone);
        ...
        if (isAllDay) {
            ...
            timezone = JalaliTime.TIMEZONE_UTC;
            startTime.hour = 0;
            startTime.minute = 0;
            startTime.second = 0;
            ...
            if (endMillis < startMillis + JalaliDateUtils.DAY_IN_MILLIS) {
                // EditEventView#fillModelFromUI() should treat this case, but we want to ensure
                // the condition anyway.
                endMillis = startMillis + JalaliDateUtils.DAY_IN_MILLIS;
            }
        } else {
     ...
     */
    private void offsetStartTimeIfNecessary(JalaliTime startTime, JalaliTime endTime, String rrule,
