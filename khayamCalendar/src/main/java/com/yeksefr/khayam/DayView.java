...
import com.yeksefr.khayam.CalendarController.EventType;
...
public class DayView extends View implements View.OnCreateContextMenuListener,
    ...
    JalaliTime mBaseDate;
    private JalaliTime mCurrentTime;
    ...
    private final Typeface mNormal = CustomFont.droidnaskh;
    private final Typeface mBold = CustomFont.droidnaskhBold;
//    private final Typeface mBold = Typeface.DEFAULT_BOLD;
    ...
    */
    ...
    private static float FARSI_TEXT_EVENT_SIZE = 14;
    ...
    public DayView(Context context, CalendarController controller,
        ...
        FARSI_TEXT_EVENT_SIZE = (int) mResources.getDimension(R.dimen.farsi_text_event_size);
    ...
    private void init(Context context) {
        ...
        mCurrentTime = new JalaliTime(Utils.getTimeZone(context, mTZUpdater));
        long currentTime = System.currentTimeMillis();
        mCurrentTime.set(currentTime);
        mTodayJulianDay = JalaliTime.getJulianDay(currentTime, mCurrentTime.gmtoff);
        ...
        for (int i = Calendar.SUNDAY; i <= Calendar.SATURDAY; i++) {
            ...
            // e.g. Tue for Tuesday
            mDayStrs[index] = JalaliDateUtils.getDayOfWeekString(mContext, i - 1, JalaliDateUtils.LENGTH_SHORT)
                    .toUpperCase();
            mDayStrs[index + 7] = mDayStrs[index];
            // e.g. Tu for Tuesday
            mDayStrs2Letter[index] = JalaliDateUtils.getDayOfWeekString(mContext, i - 1, JalaliDateUtils.LENGTH_SHORT)
                    .toUpperCase();
        ...
        mAmString = JalaliDateUtils.getAMtring(mContext);
        mPmString = JalaliDateUtils.getPMString(mContext);
        ...
        mBaseDate = new JalaliTime(Utils.getTimeZone(context, mTZUpdater));
    ...
    long getSelectedTimeInMillis() {
    	JalaliTime time = new JalaliTime(mBaseDate);
    ...
    public void setSelected(JalaliTime time, boolean ignoreTime, boolean animateToday) {
        mBaseDate.set(time);
        setSelectedHour(mBaseDate.hour);
        setSelectedEvent(null);
        mPrevSelectedEvent = null;
        long millis = mBaseDate.toMillis(false /* use isDst */);
        setSelectedDay(JalaliTime.getJulianDay(millis, mBaseDate.gmtoff));
    ...
    public JalaliTime getSelectedDay() {
    	JalaliTime time = new JalaliTime(mBaseDate);
        time.setJulianDay(mSelectionDay);
        time.hour = mSelectionHour;
    ...
    public void updateTitle() {
    	JalaliTime start = new JalaliTime(mBaseDate);
        start.normalize(true);
        JalaliTime end = new JalaliTime(start);
        end.monthDay += mNumDays - 1;
        // Move it forward one minute so the formatter doesn't lose a day
        end.minute += 1;
        end.normalize(true);

        long formatFlags = JalaliDateUtils.FORMAT_SHOW_DATE | JalaliDateUtils.FORMAT_SHOW_YEAR;
        if (mNumDays != 1) {
            // Don't show day of the month if for multi-day view
            formatFlags |= JalaliDateUtils.FORMAT_NO_MONTH_DAY;

            // Abbreviate the month if showing multiple months
            if (start.month != end.month) {
                formatFlags |= JalaliDateUtils.FORMAT_ABBREV_MONTH;
            }
        }
     ...
     */
    public int compareToVisibleTimeRange(JalaliTime time) {
        ...
        int diff = JalaliTime.compare(time, mBaseDate);
        if (diff > 0) {
            // Compare end of range
            mBaseDate.monthDay += mNumDays;
            mBaseDate.normalize(true);
            diff = JalaliTime.compare(time, mBaseDate);
    ...
    private void recalc() {
        ...
        mFirstJulianDay = JalaliTime.getJulianDay(start, mBaseDate.gmtoff);
        mLastJulianDay = mFirstJulianDay + mNumDays - 1;

//        YearMonthDate y = JalaliCalendar.gregorianToJalali(new YearMonthDate(mBaseDate.year, mBaseDate.month, mBaseDate.monthDay));
//        JalaliCalendar jc = new JalaliCalendar(y.getYear(), y.getMonth(), y.getDate());
        
//        System.out.println(y.getYear() + " " + y.getMonth() + " " + y.getDate());
//        System.out.println(jc.get(JalaliCalendar.YEAR) + " " + jc.get(JalaliCalendar.MONTH) + " " + jc.get(JalaliCalendar.DAY_OF_MONTH));
        
        mMonthLength = mBaseDate.getJalaliActualMaxMonthDays();
        mFirstVisibleDate = mBaseDate.jalaliMonthDay;
...
//        System.out.println(jc.get(JalaliCalendar.YEAR) + " " + jc.get(JalaliCalendar.MONTH) + " " + jc.get(JalaliCalendar.DAY_OF_MONTH) + " " + jc.getMaxMonthDays());
    }

    private void adjustToBeginningOfWeek(JalaliTime time) {
    ...
    private void remeasure(int width, int height) {
        ...
        mExpandAllDayRect.right = mViewWidth - Math.min((mHoursWidth - allDayIconWidth) / 2,
                EVENT_ALL_DAY_TEXT_LEFT_MARGIN);
        mExpandAllDayRect.left = Math.max(mExpandAllDayRect.right - allDayIconWidth, mViewWidth - mHoursWidth
        ...
        final long minimumDurationMillis = (long)
                (MIN_EVENT_HEIGHT * JalaliDateUtils.MINUTE_IN_MILLIS / (mCellHeight / 60.0f));
        ...
        if (mNumDays > 1) {
            ...
            if (trackBallSelection) {
                if (selectedEvent == null) {
                    ...
                    long endMillis = startMillis + JalaliDateUtils.HOUR_IN_MILLIS;
                    long extraLong = 0;
                    if (mSelectionAllday) {
...
//                break;
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if ((selectionDay < mFirstJulianDay) || (selectionDay > mLastJulianDay)) {
            ...
            JalaliTime date = view.mBaseDate;
            date.set(mBaseDate);
    ...
    private class GotoBroadcaster implements Animation.AnimationListener {
        ...
        private final JalaliTime mStart;
        private final JalaliTime mEnd;

        public GotoBroadcaster(JalaliTime start, JalaliTime end) {
            mCounter = ++sCounter;
            mStart = start;
            mEnd = end;
        }
...
//    	forward = !forward;
        ...
        final JalaliTime start = new JalaliTime(mBaseDate.timezone);
        ...
        JalaliTime newSelected = start;

        if (mNumDays == 7) {
            newSelected = new JalaliTime(start);
            adjustToBeginningOfWeek(start);
        }

        final JalaliTime end = new JalaliTime(start);
        end.monthDay += mNumDays - 1;
...
//        }
        ...
        JalaliTime weekStart = new JalaliTime(Utils.getTimeZone(mContext, mTZUpdater));
...
//        mContext.startProgressSpinner();
    ...
    private int computeDayLeftPosition(int day) {
        ...
        return mViewWidth - (day * effectiveWidth / mNumDays + mHoursWidth);
    }
    ...
    private void drawAllDayHighlights(Rect r, Canvas canvas, Paint p) {
        if (mFutureBgColor != 0) {
            ...
            if (startIndex >= 0) {
                ...
                r.top = 0;
                r.bottom = mFirstCell - 1;
                r.right = computeDayLeftPosition(startIndex) + 1;
                r.left = computeDayLeftPosition(mNumDays);
    ...
    private void drawDayHeaderLoop(Rect r, Canvas canvas, Paint p) {
        ...
        for (int day = 0; day < mNumDays; day++, cell++) {
            ...
            if (mNumDays == 1) {
                if (dayOfWeek == JalaliTime.SATURDAY) {
                    color = mWeek_saturdayColor;
                } else if (dayOfWeek == JalaliTime.SUNDAY) {
    ...
    private void drawAmPm(Canvas canvas, Paint p) {
        ...
        canvas.drawText(text, mViewWidth - HOURS_LEFT_MARGIN, y, p);
    ...
    private void drawCurrentTimeLine(Rect r, final int day, final int top, Canvas canvas,
            Paint p) {
        r.right = computeDayLeftPosition(day) + CURRENT_TIME_LINE_SIDE_BUFFER + 1;
        r.left = computeDayLeftPosition(day + 1) - CURRENT_TIME_LINE_SIDE_BUFFER + 1;
    ...
    private void drawSelectedRect(Rect r, Canvas canvas, Paint p) {
        ...
        if (mSelectionMode != SELECTION_HIDDEN && !mSelectionAllday) {
            ...
            r.left = computeDayLeftPosition(daynum + 1) + 1;
    ...
    private void drawHours(Rect r, Canvas canvas, Paint p) {
        ...
        for (int i = 0; i < 24; i++) {
            ...
            canvas.drawText(Helper.convertEnglishNumbersToParsi(time), mViewWidth - HOURS_LEFT_MARGIN, y, p);
...
//        p.setTypeface(Typeface.DEFAULT);
    ...
    private void drawDayHeader(String dayStr, int day, int cell, Canvas canvas, Paint p) {
        ...
        if (mNumDays > 1) {
            ...
            p.setTypeface(todayIndex == day ? mBold : mNormal);
            canvas.drawText(Helper.convertEnglishNumbersToParsi(dateNumStr), x, y, p);
    ...
    private void drawGridBackground(Rect r, Canvas canvas, Paint p) {
        ...
        final float stopX = mViewWidth - computeDayLeftPosition(mNumDays);
        float y = 0;
    ...
    private void drawBgColors(Rect r, Canvas canvas, Paint p) {
        ...
        if (mNumDays == 1 && todayIndex == 0) {
            ...
            if (lineY < mViewStartY + mViewHeight) {
                ...
                r.right = computeDayLeftPosition(todayIndex);
                r.top = lineY;
                r.bottom = mViewStartY + mViewHeight;
                p.setColor(mFutureBgColor);
                canvas.drawRect(r, p);
            ...
            // Future
            r.right = computeDayLeftPosition(0) + 1;
            r.left = computeDayLeftPosition(mNumDays);
            r.top = mDestRect.top;
            r.bottom = mDestRect.bottom;
            p.setColor(mFutureBgColor);
...
//            paint.setTypeface(CustomFont.droidnaskh);
            paint.setTypeface(mNormal);
            paint.setTextSize(FARSI_TEXT_EVENT_SIZE);
            ...
            layout = new StaticLayout(bob, 0, bob.length(), new TextPaint(paint), r.width(),
                    Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false, null, r.width());
    ...
    private void drawAllDayEvents(int firstDay, int numDays, Canvas canvas, Paint p) {
        ...
        for (int i = 0; i < numEvents; i++) {
            ...
            // event rectangle.
            event.right = computeDayLeftPosition(startIndex);
            event.left = computeDayLeftPosition(endIndex + 1) + DAY_GAP;
    ...
    private void drawEvents(int date, int dayIndex, int top, Canvas canvas, Paint p) {
        Paint eventTextPaint = mEventTextPaint;
        int left = computeDayLeftPosition(dayIndex + 1) + 1;
        int cellWidth = computeDayLeftPosition(dayIndex) - left + 1;
...
//    }
    ...
    private void updateEventDetails() {
        ...
        CustomTextView titleView = (CustomTextView) mPopupView.findViewById(R.id.event_title);
        ...
        int flags;
        if (event.allDay) {
            flags = JalaliDateUtils.FORMAT_UTC | JalaliDateUtils.FORMAT_SHOW_DATE
                    | JalaliDateUtils.FORMAT_SHOW_WEEKDAY | JalaliDateUtils.FORMAT_ABBREV_ALL;
        } else {
            flags = JalaliDateUtils.FORMAT_SHOW_TIME | JalaliDateUtils.FORMAT_SHOW_DATE
                    | JalaliDateUtils.FORMAT_SHOW_WEEKDAY | JalaliDateUtils.FORMAT_ABBREV_ALL
                    | JalaliDateUtils.FORMAT_CAP_NOON_MIDNIGHT;
        }
        if (DateFormat.is24HourFormat(mContext)) {
            flags |= JalaliDateUtils.FORMAT_24HOUR;
        }
        String timeRange = Utils.formatDateRange(mContext, event.startMillis, event.endMillis,
                flags);
        CustomTextView timeView = (CustomTextView) mPopupView.findViewById(R.id.time);
        timeView.setText(timeRange);

        CustomTextView whereView = (CustomTextView) mPopupView.findViewById(R.id.where);
    ...
    private void doSingleTapUp(MotionEvent ev) {
        ...
        if (mMaxAlldayEvents > mMaxUnexpandedAlldayEventCount) {
            ...
            if((x > mViewWidth - mHoursWidth && y > DAY_HEADER_HEIGHT && y < DAY_HEADER_HEIGHT + mAlldayHeight)
        ...
        if (!validPosition) {
            if (y < DAY_HEADER_HEIGHT) {
            	JalaliTime selectedTime = new JalaliTime(mBaseDate);
...
//        int flags = JalaliDateUtils.FORMAT_SHOW_TIME
//                | JalaliDateUtils.FORMAT_CAP_NOON_MIDNIGHT
//                | JalaliDateUtils.FORMAT_SHOW_WEEKDAY;
        final String title = JalaliDateUtils.getTimeAndWeekday(mContext, startMillis);
    ...
    private boolean setSelectionFromPosition(int x, final int y, boolean keepOldSelection) {
        ...
        x = mViewWidth - mHoursWidth - x;
        if (x < 0) {
            x = 0;
        }

        int day = x / (mCellWidth + DAY_GAP);
        if (day >= mNumDays) {
            day = mNumDays - 1;
        }
        day += mFirstJulianDay;
...
//        }
    ...
    private void findSelectedEvent(int x, int y) {
        ...
        int numEvents = events.size();
        int left = computeDayLeftPosition(mSelectionDay - mFirstJulianDay + 1);
        int top = 0;
        setSelectedEvent(null);
    ...
    public boolean onLongClick(View v) {
        int flags = JalaliDateUtils.FORMAT_SHOW_WEEKDAY;
        long time = getSelectedTimeInMillis();
        if (!mSelectionAllday) {
            flags |= JalaliDateUtils.FORMAT_SHOW_TIME;
        }
        if (DateFormat.is24HourFormat(mContext)) {
            flags |= JalaliDateUtils.FORMAT_24HOUR;
        }
