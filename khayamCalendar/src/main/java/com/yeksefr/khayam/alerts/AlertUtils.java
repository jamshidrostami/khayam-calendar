...
package com.yeksefr.khayam.alerts;
...
import com.yeksefr.khayam.EventInfoActivity;
...
public class AlertUtils {
    ...
    static String formatTimeLocation(Context context, long startMillis, boolean allDay,
        ...
        time.setToNow();
        int today = JalaliTime.getJulianDay(time.toMillis(false), time.gmtoff);
        time.set(startMillis);
        int eventDay = JalaliTime.getJulianDay(time.toMillis(false), allDay ? 0 : time.gmtoff);
        ...
        if (!allDay) {
            ...
            flags |= JalaliDateUtils.FORMAT_UTC;
        }
        ...
        if (!allDay && tz != JalaliTime.getCurrentTimezone()) {
    ...
    static void flushOldAlertsFromInternalStorage(Context context) {
        if (BYPASS_DB) {
            ...
            if (nowTime - lastFlushTimeMs > FLUSH_INTERVAL_MS) {
                ...
                SharedPreferences.Editor editor = prefs.edit();
                JalaliTime timeObj = new JalaliTime();
    ...
    private static int getIntervalInDays(long startMillis, long endMillis, JalaliTime timeObj) {
        timeObj.set(startMillis);
        int startDay = JalaliTime.getJulianDay(startMillis, timeObj.gmtoff);
        timeObj.set(endMillis);
        return JalaliTime.getJulianDay(endMillis, timeObj.gmtoff) - startDay;
    }
}
