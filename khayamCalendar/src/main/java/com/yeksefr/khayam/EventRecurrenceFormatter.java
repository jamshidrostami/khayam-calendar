...
public class EventRecurrenceFormatter
{
    ...
    public static String getRepeatString(Context context, Resources r, EventRecurrence recurrence,
        ...
        if (includeEndString) {
            ...
            if (recurrence.until != null) {
                try {
                	JalaliTime t = new JalaliTime();
                    t.parse(recurrence.until);
                    final String dateStr = JalaliDateUtils.formatDateTime(context,
                            t.toMillis(false), JalaliDateUtils.FORMAT_NUMERIC_DATE);
        ...
        switch (recurrence.freq) {
            case EventRecurrence.DAILY:
                return r.getQuantityString(R.plurals.recurrence_interval_daily, interval, interval) + endString;
            case EventRecurrence.WEEKLY: {
                if (recurrence.repeatsOnEveryWeekDay()) {
                    ...
                    int dayOfWeekLength = JalaliDateUtils.LENGTH_SHORT;
                    if (recurrence.bydayCount == 1) {
                        dayOfWeekLength = JalaliDateUtils.LENGTH_LONG;
                    }
                    ...
                    if (recurrence.bydayCount > 0) {
                        ...
                        for (int i = 0 ; i < count ; i++) {
                            days.append(dayToString(context, recurrence.byday[i], dayOfWeekLength));
                            days.append(", ");
                        }
                        days.append(dayToString(context, recurrence.byday[count], dayOfWeekLength));
                        ...
                        string = dayToString(context, day, JalaliDateUtils.LENGTH_LONG);
                    }
     ...
     */
    private static String dayToString(Context context, int day, int dayOfWeekLength) {
        return JalaliDateUtils.getDayOfWeekString(context, dayToUtilDay(day) - 1, dayOfWeekLength);
    }
