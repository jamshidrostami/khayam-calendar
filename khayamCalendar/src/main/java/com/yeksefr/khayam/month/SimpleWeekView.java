...
public class SimpleWeekView extends View {
    ...
    protected static final int DEFAULT_WEEK_START = JalaliTime.SATURDAY;
    ...
    protected String[] mMiladiDayNumbers;
    ...
    // is)
    protected String mTimeZone = JalaliTime.getCurrentTimezone();
    ...
    public void setWeekParams(HashMap<String, Integer> params, String tz) {
        ...
        mMiladiDayNumbers = new String[mNumCells];
        ...
        JalaliTime time = new JalaliTime(tz);
        time.setJulianDay(julianMonday);
        
        // If we're showing the week number calculate it based on Monday
        int i = 0;
        if (mShowWeekNum) {
            mDayNumbers[0] = Helper.convertEnglishNumbersToParsi(Integer.toString(time.getJalaliWeekNumber()));
            mMiladiDayNumbers[0] = Integer.toString(time.getWeekNumber());
            i++;
        }
        ...
        mFirstJulianDay = JalaliTime.getJulianDay(t, time.gmtoff);
        mFirstMonth = time.jalaliMonth;

        // Figure out what day today is
        JalaliTime today = new JalaliTime(tz);
        today.setToNow();
        mHasToday = false;
        mToday = -1;
        ...
        for (; i < mNumCells; i++) {
            if (time.jalaliMonthDay == 1) {
                mFirstMonth = time.jalaliMonth;
            }
            mOddMonth [i] = (time.jalaliMonth %2) == 1;
            if (time.jalaliMonth == focusMonth) {
                mFocusDay[i] = true;
            } else {
                mFocusDay[i] = false;
            }
            ...
            mMiladiDayNumbers[i] = Integer.toString(time.monthDay);
            
            mDayNumbers[i] = Helper.convertEnglishNumbersToParsi(Integer.toString(time.jalaliMonthDay));
            time.addDay();
        }
    ...
    public JalaliTime getDayFromLocation(float x) {
        ...
        if (x < mPadding || x > mWidth - mPadding - dayStart) {
            return null;
        }
        // Selection is (x - start) / (pixels/day) == (x -s) * day / pixels
//        int dayPosition = (int) ((x - dayStart) * mNumDays / (mWidth - dayStart - mPadding));
        int dayPosition = (int) ((mWidth - dayStart - x) * mNumDays / (mWidth - dayStart - mPadding));
    ...
    protected void drawWeekNums(Canvas canvas) {
        ...
        if (mShowWeekNum) {
            ...
            p.setTypeface(CustomFont.droidnaskh);
            int x = mWidth - (mWidth - mPadding * 2) / divisor + mPadding;
        ...
        for (; i < nDays; i++) {
            ...
            int x = mWidth - (2 * i + 1) * (mWidth - mPadding * 2) / (divisor) + mPadding;
    ...
    protected void drawDaySeparators(Canvas canvas) {
        ...
        if (mShowWeekNum) {
            ...
            int x = mWidth - (mWidth - mPadding * 2) / mNumCells + mPadding;
...
//            }
            ...
            selectedPosition = 7 - selectedPosition;
            
            mSelectedLeft = (selectedPosition - 1) * (mWidth - mPadding * 2) / mNumCells
                    + mPadding;
            mSelectedRight = (selectedPosition) * (mWidth - mPadding * 2) / mNumCells
                    + mPadding;
        }
    }
    ...
    JalaliTime mLastHoverTime = null;
}