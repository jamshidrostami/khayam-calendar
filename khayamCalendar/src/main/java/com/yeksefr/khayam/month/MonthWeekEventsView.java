...
package com.yeksefr.khayam.month;

import com.yeksefr.khayam.R;
import com.yeksefr.khayam.CustomFont;
import com.yeksefr.khayam.Event;
...
public class MonthWeekEventsView extends SimpleWeekView {
    ...
    protected JalaliTime mToday = new JalaliTime();
    ...
    protected TextPaint mEventExtrasPaint2;
    ...
    public void createDna(ArrayList<Event> unsortedEvents) {
        ...
        if (!mShowDetailsInMonth) {
            ...
            for (int day = 0; day < numDays; day++) {
                mDayXs[day] = computeDayLeftPosition(day + 2) + DNA_WIDTH / 2 + DNA_SIDE_PADDING;
...
//        mMonthNumPaint.setTypeface(Typeface.DEFAULT);
        ...
        mEventPaint.setTypeface(CustomFont.droidnaskh);
        ...
        mEventExtrasPaint.setTypeface(CustomFont.droidnaskh);
        ...
        mEventExtrasPaint2 = new TextPaint(mEventExtrasPaint);
        mEventExtrasPaint2.setTextAlign(Align.RIGHT);
        ...
        mWeekNumPaint.setTextAlign(Align.LEFT);
        mWeekNumPaint.setTypeface(CustomFont.droidnaskh);
    ...
    public boolean updateToday(String tz) {
        ...
        int julianToday = JalaliTime.getJulianDay(mToday.toMillis(false), mToday.gmtoff);
    ...
    protected void drawToday(Canvas canvas) {
        ...
        r.left = computeDayLeftPosition(mTodayIndex + 2) + (TODAY_HIGHLIGHT_WIDTH / 2);
    ...
    // Computes the x position for the left w21side of the given day
    private int computeDayLeftPosition(int day) {
        ...
        x = mWidth - ((day - 1) * effectiveWidth / mNumDays + xOffset);
        return x;
    }
    ...
    protected void drawDaySeparators(Canvas canvas) {
        ...
        while (i < count) {
            int x = computeDayLeftPosition(i / 4 - wkNumOffset + 1);
            lines[i++] = x;
            lines[i++] = y0;
            lines[i++] = x;
            lines[i++] = y1;
...
//            r.left = 0;
            r.right = mWidth;
            r.left = computeDayLeftPosition(i - offset + 1);
            ...
            i++;
            // compute left edge for i, set up r, draw
            r.right = computeDayLeftPosition(i - offset + 1);
            r.left = 0;
//            r.right = mWidth;
//            r.left = computeDayLeftPosition(i - offset);
        ...
        if (mHasToday) {
            ...
            r.left = computeDayLeftPosition(mTodayIndex + 2);
            r.right = computeDayLeftPosition(mTodayIndex + 1);
            canvas.drawRect(r, p);
        }
    }
    ...
    private void drawClick(Canvas canvas) {
        if (mClickedDayIndex != -1) {
            ...
            r.left = computeDayLeftPosition(mClickedDayIndex + 1);
            r.right = computeDayLeftPosition(mClickedDayIndex);
            r.top = DAY_SEPARATOR_INNER_WIDTH;
            r.bottom = mHeight;
            canvas.drawRect(r, p);
            p.setAlpha(alpha);
    ...
    protected void drawWeekNums(Canvas canvas) {
        ...
        if (mShowWeekNum) {
            x = mWidth - (SIDE_PADDING_WEEK_NUMBER + mPadding);
        ...
        for (; i < numCount; i++) {
            ...
            x = computeDayLeftPosition(i - offset + 1) + (SIDE_PADDING_MONTH_NUMBER);
            y = (int) (mHeight  + ((mMonthNumPaint.descent() + mMonthNumPaint.ascent()) / 2));
            mMonthNumPaint.setTextAlign(Align.LEFT);
            canvas.drawText(mMiladiDayNumbers[i], x, y, mMonthNumPaint);
    ...
    protected void drawEvents(Canvas canvas) {
        ...
        for (ArrayList<Event> eventDay : mEvents) {
            ...
            int ySquare;
            int xSquare = computeDayLeftPosition(day + 2) + SIDE_PADDING_MONTH_NUMBER + 1;
            ...
            if (remaining > 0) {
                xSquare = computeDayLeftPosition(day + 1) - SIDE_PADDING_MONTH_NUMBER - 1;
    ...
    protected int drawEvent(Canvas canvas, Event event, int x, int y, int rightEdge,
        ...
        if (showTimes && !allDay) {
            ...
            text = JalaliDateUtils.formatDateRange(getContext(), mFormatter, event.startMillis,
                    event.endMillis, JalaliDateUtils.FORMAT_SHOW_TIME | JalaliDateUtils.FORMAT_ABBREV_ALL,
    ...
    protected void drawMoreEvents(Canvas canvas, int remainingEvents, int x) {
        ...
        mEventExtrasPaint2.setAntiAlias(true);
        mEventExtrasPaint2.setFakeBoldText(true);
        canvas.drawText(String.format(text, remainingEvents), x, y, mEventExtrasPaint2);
        mEventExtrasPaint2.setFakeBoldText(false);
    }
...
//        return ((int) ((x - dayStart) * mNumDays / (mWidth - dayStart - mPadding)));
        return ((int) ((mWidth - x - dayStart) * mNumDays / (mWidth - dayStart - mPadding)));
    }

    @Override
    public JalaliTime getDayFromLocation(float x) {
        ...
        if (mWeek == 0) {
            // This week is weird...
            if (day < JalaliTime.EPOCH_JULIAN_DAY) {
                day++;
            } else if (day == JalaliTime.EPOCH_JULIAN_DAY) {
                time.set(1, 0, 1970);
                time.normalize(true);
                return time;
            }
    ...
    public void setClickedDay(float xLocation) {
        mClickedDayIndex = getDayIndexFromLocation(xLocation) + 1;
        invalidate();
    }
