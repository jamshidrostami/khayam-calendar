...
import com.yeksefr.khayam.event.CreateEventDialogFragment;
...
public class MonthByWeekFragment extends SimpleDayPickerFragment implements
    ...
    private final JalaliTime mDesiredDay = new JalaliTime();
    ...
    private Handler mEventDialogHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (manager != null) {
                JalaliTime day = (JalaliTime) msg.obj;
    ...
    private void updateLoadedDays() {
        ...
        mTempTime.set(first);
        mFirstLoadedJulianDay = JalaliTime.getJulianDay(first, mTempTime.gmtoff);
        mTempTime.set(last);
        mLastLoadedJulianDay = JalaliTime.getJulianDay(last, mTempTime.gmtoff);
    }
    ...
    protected void setUpAdapter() {
        ...
        weekParams.put(SimpleWeeksAdapter.WEEK_PARAMS_JULIAN_DAY,
        		JalaliTime.getJulianDay(mSelectedDay.toMillis(true), mSelectedDay.gmtoff));
    ...
    public void handleEvent(EventInfo event) {
        if (event.eventType == EventType.GO_TO) {
            ...
            if (mDaysPerWeek * mNumWeeks * 2 < Math.abs(
            		JalaliTime.getJulianDay(event.selectedTime.toMillis(true), event.selectedTime.gmtoff)
                    - JalaliTime.getJulianDay(mFirstVisibleDay.toMillis(true), mFirstVisibleDay.gmtoff)
    ...
    protected void setMonthDisplayed(JalaliTime time, boolean updateHighlight) {
        ...
        if (!mIsMiniMonth) {
            ...
            if (newTime != controller.getTime() && mUserScrolled) {
                long offset = useSelected ? 0 : JalaliDateUtils.WEEK_IN_MILLIS * mNumWeeks / 3;
                controller.setTime(newTime + offset);
            }
            controller.sendEvent(this, EventType.UPDATE_TITLE, time, time, time, -1,
                    ViewType.CURRENT, JalaliDateUtils.FORMAT_SHOW_DATE | JalaliDateUtils.FORMAT_NO_MONTH_DAY
                            | JalaliDateUtils.FORMAT_SHOW_YEAR, null, null);
        }
    }
