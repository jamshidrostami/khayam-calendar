...
import com.yeksefr.khayam.CalendarController.EventHandler;
import com.yeksefr.khayam.CalendarController.EventInfo;
import com.yeksefr.khayam.CalendarController.EventType;
import com.yeksefr.khayam.CalendarController.ViewType;
import com.yeksefr.khayam.agenda.AgendaFragment;
import com.yeksefr.khayam.month.MonthByWeekFragment;
import com.yeksefr.khayam.selectcalendars.SelectVisibleCalendarsFragment;
...
public class AllInOneActivity extends AbstractCalendarActivity implements EventHandler,
    ...
    private CustomTextView mHomeTime;
    private CustomTextView mDateRange;
    private CustomTextView mWeekCustomTextView;
    ...
    protected void onCreate(Bundle icicle) {
        ...
        if (mIsTabletConfig) {
            mDateRange = (CustomTextView) findViewById(R.id.date_bar);
            mDateRange.setTypeface(CustomFont.droidnaskh);
            mWeekCustomTextView = (CustomTextView) findViewById(R.id.week_num);
        } else {
            mDateRange = (CustomTextView) getLayoutInflater().inflate(R.layout.date_range_title, null);
        }
        ...
        mHomeTime = (CustomTextView) findViewById(R.id.home_time);
    ...
    private void initFragments(long timeMillis, int viewType, Bundle icicle) {
        ...
        if (viewType == ViewType.EDIT) {
            ...
            info = new EventInfo();
            if (end != -1) {
                info.endTime = new JalaliTime();
                info.endTime.set(end);
            }
            if (begin != -1) {
                info.startTime = new JalaliTime();
    ...
    private void setTitleInActionBar(EventInfo event) {
        ...
        final String msg = JalaliDateUtils.formatDateRange(this, start, end, (int) event.extraLong);
    ...
    private void updateSecondaryTitleFields(long visibleMillisSinceEpoch) {
        ...
        if (visibleMillisSinceEpoch != -1) {
            int weekNum = Utils.getJalaliWeekNumberFromTime(visibleMillisSinceEpoch, this);
            mWeekNum = weekNum;
        }

        if (visibleMillisSinceEpoch != -1 && mCurrentView == ViewType.MONTH && mIsTabletConfig && mWeekCustomTextView != null) {
            String monthString = buildGregorianMonth(visibleMillisSinceEpoch);
            mWeekCustomTextView.setText(monthString);
            mWeekCustomTextView.setVisibility(View.VISIBLE);
        } else if (mShowWeekNum && (mCurrentView == ViewType.WEEK) && mIsTabletConfig
            		&& mWeekCustomTextView != null) {
            	String weekString = Helper.convertEnglishNumbersToParsi(getResources().getQuantityString(R.plurals.weekN, mWeekNum,
            			mWeekNum));
            	mWeekCustomTextView.setText(weekString);
            	mWeekCustomTextView.setVisibility(View.VISIBLE);
        } else if (visibleMillisSinceEpoch != -1 && mWeekCustomTextView != null
                && mCurrentView == ViewType.DAY && mIsTabletConfig) {
        	JalaliTime time = new JalaliTime(mTimeZone);
            time.set(visibleMillisSinceEpoch);
            int julianDay = JalaliTime.getJulianDay(visibleMillisSinceEpoch, time.gmtoff);
            time.setToNow();
            int todayJulianDay = JalaliTime.getJulianDay(time.toMillis(false), time.gmtoff);
            String dayString = Utils.getDayOfWeekString(julianDay, todayJulianDay,
                    visibleMillisSinceEpoch, this);
            mWeekCustomTextView.setText(dayString);
            mWeekCustomTextView.setVisibility(View.VISIBLE);
        } else if (visibleMillisSinceEpoch != -1 && mWeekCustomTextView != null && (!mIsTabletConfig || mCurrentView != ViewType.DAY)) {
            mWeekCustomTextView.setVisibility(View.GONE);
        }

        if (mHomeTime != null
                && (mCurrentView == ViewType.DAY || mCurrentView == ViewType.WEEK
                        || mCurrentView == ViewType.AGENDA)
                && !TextUtils.equals(mTimeZone, JalaliTime.getCurrentTimezone())) {
        	JalaliTime time = new JalaliTime(mTimeZone);
            time.setToNow();
            ...
            mHomeTime.postDelayed(
                    mHomeTimeUpdater,
                    JalaliDateUtils.MINUTE_IN_MILLIS - (millis % JalaliDateUtils.MINUTE_IN_MILLIS));
        } else if (mHomeTime != null) {
            mHomeTime.setVisibility(View.GONE);
        }
    }

    private String buildGregorianMonth(long millis) {
        ...
        String date = DateUtils.formatDateRange(getBaseContext(), monthStartTime,
        		monthEndTime, flags).toString();
         return date;
    }
