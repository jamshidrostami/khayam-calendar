...
package com.yeksefr.khayam.recurrencepicker;
...
public class RecurrencePickerDialog extends DialogFragment implements OnItemSelectedListener,
    ...
    private class RecurrenceModel implements Parcelable {
        ...
        public void writeToParcel(Parcel dest, int flags) {
            ...
            dest.writeInt(end);
            dest.writeInt(endDate.jalaliYear);
            dest.writeInt(endDate.jalaliMonth);
            dest.writeInt(endDate.jalaliMonthDay);
    ...
    private JalaliTime mTime = new JalaliTime(); // TODO timezone?
    ...
    private CustomTextView mIntervalPreText;
    private CustomTextView mIntervalPostText;

    private int mIntervalResId = -1;

    private Spinner mEndSpinner;
    private CustomTextView mEndDateTextView;
    private EditText mEndCount;
    private CustomTextView mPostEndCount;
    ...
    static private void copyModelToEventRecurrence(final RecurrenceModel model,
        ...
        switch (model.end) {
            case RecurrenceModel.END_BY_DATE:
                if (model.endDate != null) {
                    model.endDate.switchTimezone(JalaliTime.TIMEZONE_UTC);
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mIntervalPreText = (CustomTextView) mView.findViewById(R.id.intervalPreText);
        mIntervalPostText = (CustomTextView) mView.findViewById(R.id.intervalPostText);
        ...
        mPostEndCount = (CustomTextView) mView.findViewById(R.id.postEndCount);

        mEndDateTextView = (CustomTextView) mView.findViewById(R.id.endDate);
        mEndDateTextView.setOnClickListener(this);
        if (mModel.endDate == null) {
            mModel.endDate = new JalaliTime(mTime);
            switch (mModel.freq) {
                case RecurrenceModel.FREQ_DAILY:
                case RecurrenceModel.FREQ_WEEKLY:
                    mModel.endDate.addJalaliMonth(1);
                    break;
                case RecurrenceModel.FREQ_MONTHLY:
                	mModel.endDate.addJalaliMonth(3);
                    break;
                case RecurrenceModel.FREQ_YEARLY:
                    mModel.endDate.jalaliYear += 3;
                    mModel.endDate.jalaliToGregorian();
                    break;
            }
...
//        String[] dayOfWeekString = new DateFormatSymbols().getWeekdays();
        String[] dayOfWeekString = mResources.getStringArray(R.array.shortWeekdayNames);
...
//        dayOfWeekString = new DateFormatSymbols().getShortWeekdays();
        dayOfWeekString = mResources.getStringArray(R.array.shortWeekdayNames);
        ...
        for (int i = 0; i < 7; i++) {
            ...
            mWeekByDayButtons[idx].setTextOff(dayOfWeekString[TIME_DAY_TO_CALENDAR_DAY[idx] - 1]);
            mWeekByDayButtons[idx].setTextOn(dayOfWeekString[TIME_DAY_TO_CALENDAR_DAY[idx] - 1]);
    ...
    public void updateDialog() {
        ...
        if (mModel.end == RecurrenceModel.END_BY_DATE) {
            final String dateStr = JalaliDateUtils.formatDateTime(getActivity(),
                    mModel.endDate.toMillis(false), JalaliDateUtils.FORMAT_NUMERIC_DATE);
    ...
    public void onDateSet(DatePickerDialog view, int year, int monthOfYear, int dayOfMonth) {
        if (mModel.endDate == null) {
            mModel.endDate = new JalaliTime(mTime.timezone);
            mModel.endDate.hour = mModel.endDate.minute = mModel.endDate.second = 0;
        }
        mModel.endDate.jalaliYear = year;
        mModel.endDate.jalaliMonth = monthOfYear;
        mModel.endDate.jalaliMonthDay = dayOfMonth;
        mModel.endDate.jalaliToGregorian();
        updateDialog();
    }
...
//            bundle.putInt("min", Utils.YEAR_MIN);
//            bundle.putInt("max", Utils.YEAR_MAX);
//            bundle.putInt("year", mModel.endDate.jalaliYear);
//            bundle.putInt("month", mModel.endDate.jalaliMonth);
//            bundle.putInt("day", mModel.endDate.jalaliMonthDay);
 ...
 // Implements OnClickListener interface
    ...
    public void onClick(View v) {
        if (mEndDateTextView == v) {
            ...
            mDatePickerDialog = DatePickerDialog.newInstance(this, mModel.endDate.jalaliYear,
                    mModel.endDate.jalaliMonth, mModel.endDate.jalaliMonthDay);
//            mDatePickerDialog.setFirstDayOfWeek(Utils.getFirstDayOfWeekAsCalendar(getActivity()));
            mDatePickerDialog.setYearRange(Utils.YEAR_MIN, Utils.YEAR_MAX);
            mDatePickerDialog.show(getFragmentManager(), FRAG_TAG_DATE_PICKER);
        } else if (mDone == v) {
...
//    }
    ...
    private class EndSpinnerAdapter extends ArrayAdapter<CharSequence> {
        ...
        /**
         * @param context
         * @param CustomTextViewResourceId
         * @param objects
         */
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            CustomTextView item = (CustomTextView) v.findViewById(R.id.spinner_item);
            int markerStart;
            switch (position) {
                case RecurrenceModel.END_NEVER:
