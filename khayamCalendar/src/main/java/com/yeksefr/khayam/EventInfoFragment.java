...
import static com.yeksefr.khayam.CalendarController.EVENT_EDIT_ON_LAUNCH;
...
import com.yeksefr.khayam.alerts.QuickResponseActivity;
import com.yeksefr.khayam.event.AttendeesView;
import com.yeksefr.khayam.event.EditEventActivity;
import com.yeksefr.khayam.event.EditEventHelper;
import com.yeksefr.khayam.event.EventColorPickerDialog;
import com.yeksefr.khayam.event.EventViewUtils;
...
public class EventInfoFragment extends DialogFragment implements OnCheckedChangeListener,
    ...
    private CustomTextView mWhenDateTime;
    private CustomTextView mWhere;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mTitle = (CustomTextView) mView.findViewById(R.id.title);
        mWhenDateTime = (CustomTextView) mView.findViewById(R.id.when_datetime);
        mWhere = (CustomTextView) mView.findViewById(R.id.where);
    ...
    private void updateEvent(View view) {
        ...
        if (!TextUtils.isEmpty(rRule)) {
            ...
            JalaliTime date = new JalaliTime(localTimezone);
            date.set(mStartMillis);
            if (mAllDay) {
                date.timezone = JalaliTime.TIMEZONE_UTC;
            }
        ...
        if (location == null || location.trim().length() == 0) {
            ...
            final CustomTextView CustomTextView = mWhere;
            if (CustomTextView != null) {
                CustomTextView.setAutoLinkMask(0);
                CustomTextView.setText(location.trim());
                try {
                    CustomTextView.setText(Utils.extendedLinkify(CustomTextView.getText().toString(), true));

                    // Linkify.addLinks() sets the CustomTextView movement method if it finds any links.
                    // We must do the same here, in case linkify by itself did not find any.
                    // (This is cloned from Linkify.addLinkMovementMethod().)
                    MovementMethod mm = CustomTextView.getMovementMethod();
                    if ((mm == null) || !(mm instanceof LinkMovementMethod)) {
                        if (CustomTextView.getLinksClickable()) {
                ...
                CustomTextView.setOnTouchListener(new OnTouchListener() {
                    @Override
    ...
    private void setTextCommon(View view, int id, CharSequence text) {
        CustomTextView CustomTextView = (CustomTextView) view.findViewById(id);
        if (CustomTextView == null)
            return;
