...
public class RecurrenceProcessor
{
    ...
    private JalaliTime mIterator = new JalaliTime(JalaliTime.TIMEZONE_UTC);
    private JalaliTime mUntil = new JalaliTime(JalaliTime.TIMEZONE_UTC);
    private StringBuilder mStringBuilder = new StringBuilder();
    private JalaliTime mGenerated = new JalaliTime(JalaliTime.TIMEZONE_UTC);
     ...
     */
    public long getLastOccurence(JalaliTime dtstart,
     ...
     * @throws DateException
     */
    public long getLastOccurence(JalaliTime dtstart, JalaliTime maxtime,
                                 RecurrenceSet recur) throws DateException {
        long lastTime = -1;
     ...
     */
    private static int filter(EventRecurrence r, JalaliTime iterator)
    {
        boolean found;
        int freq = r.freq;
        ...
        if (EventRecurrence.WEEKLY >= freq) {
            ...
            if (r.byweeknoCount > 0) {
                found = listContains(r.byweekno, r.byweeknoCount,
                                iterator.getJalaliWeekNumber(),
                                iterator.getJalaliActualMaximum(JalaliTime.WEEK_NUM));
                if (!found) {
                    return 2;
                }
            }
        ...
        if (EventRecurrence.DAILY >= freq) {
            ...
            if (r.byyeardayCount > 0) {
                found = listContains(r.byyearday, r.byyeardayCount,
                                iterator.jalaliYearDay, iterator.getJalaliActualMaximum(JalaliTime.YEAR_DAY));
                if (!found) {
                    return 3;
                }
            }
            ...
            if (r.bymonthdayCount > 0 ) {
                found = listContains(r.bymonthday, r.bymonthdayCount,
                                iterator.jalaliMonthDay,
                                iterator.getJalaliActualMaximum(JalaliTime.MONTH_DAY));
                if (!found) {
                    return 4;
                }
            }
...
byday:
        ...
        if (EventRecurrence.HOURLY >= freq) {
            ...
            found = listContains(r.byhour, r.byhourCount,
                            iterator.hour,
                            iterator.getJalaliActualMaximum(JalaliTime.HOUR));
            if (!found) {
                return 6;
            }
        }
        if (EventRecurrence.MINUTELY >= freq) {
            ...
            found = listContains(r.byminute, r.byminuteCount,
                            iterator.minute,
                            iterator.getJalaliActualMaximum(JalaliTime.MINUTE));
            if (!found) {
                return 7;
            }
        }
        if (EventRecurrence.SECONDLY >= freq) {
            ...
            found = listContains(r.bysecond, r.bysecondCount,
                            iterator.second,
                            iterator.getJalaliActualMaximum(JalaliTime.SECOND));
            if (!found) {
                return 8;
            }
        }
...
bysetpos:
     ...
     */
    private static boolean filterMonthlySetPos(EventRecurrence r, JalaliTime instance) {
        /*
         ...
         */
        int dotw = (instance.weekDay - instance.jalaliMonthDay + 36) % 7;
         ...
         */
        int maxDay = instance.getJalaliActualMaximum(JalaliTime.MONTH_DAY);
        ...
        for (int i = r.bysetposCount - 1; i >= 0; i--) {
            ...
            if (index > 0) {
                ...
                if (daySet[index-1] == instance.jalaliMonthDay) {
                    return true;
                }
            } else if (index < 0) {
                ...
                if (daySet[daySetLength + index] == instance.jalaliMonthDay) {
                    return true;
                }
            } else {
    ...
    public static class DaySet
    {
        ...
        {
            mTime = new JalaliTime(JalaliTime.TIMEZONE_UTC);
        }
        ...
        boolean get(JalaliTime iterator, int day)
        {
            int realYear = iterator.jalaliYear;
            int realMonth = iterator.jalaliMonth;

            JalaliTime t = null;

            if (SPEW) {
                Log.i(TAG, "get called with iterator=" + iterator
                        + " " + iterator.jalaliMonth
                        + "/" + iterator.jalaliMonthDay
                        + "/" + iterator.jalaliYear + " day=" + day);
            }
            if (day < 1 || day > 28) {
...
//                unsafeNormalize(t);
                t.normalize(true);
                realYear = t.jalaliYear;
                realMonth = t.jalaliMonth;
                day = t.jalaliMonthDay;
                if (SPEW) {
                    Log.i(TAG, "normalized t=" + t + " " + t.jalaliMonth
                            + "/" + t.jalaliMonthDay
                            + "/" + t.jalaliYear);
                }
            }
...
//                    unsafeNormalize(t);
                    t.normalize(true);
                    if (SPEW) {
                        Log.i(TAG, "set t=" + t + " " + t.jalaliMonth
                                + "/" + t.jalaliMonthDay
                                + "/" + t.jalaliYear
         ...
         */
        private static int generateDaysList(JalaliTime generated, EventRecurrence r)
        {
            int days = 0;
            ...
            lastDayThisMonth = generated.getJalaliActualMaximum(JalaliTime.MONTH_DAY);

            // BYDAY
            count = r.bydayCount;
            if (count > 0) {
                // calculate the day of week for the first of this month (first)
                j = generated.jalaliMonthDay;
                while (j >= 8) {
                    j -= 7;
                }
     ...
     */
    public long[] expand(JalaliTime dtstart,
            RecurrenceSet recur,
     ...
     */
    public void expand(JalaliTime dtstart,
            EventRecurrence r,
...
//        unsafeNormalize(dtstart);
        ...
        JalaliTime iterator = mIterator;
        JalaliTime until = mUntil;
        StringBuilder sb = mStringBuilder;
        JalaliTime generated = mGenerated;
        DaySet days = mDays;
        ...
        try {
            ...
            switch (freq)
            {
                case EventRecurrence.SECONDLY:
                    freqField = JalaliTime.SECOND;
                    break;
                case EventRecurrence.MINUTELY:
                    freqField = JalaliTime.MINUTE;
                    break;
                case EventRecurrence.HOURLY:
                    freqField = JalaliTime.HOUR;
                    break;
                case EventRecurrence.DAILY:
                    freqField = JalaliTime.MONTH_DAY;
                    break;
                ...
                case EventRecurrence.MONTHLY:
                    freqField = JalaliTime.MONTH;
                    break;
                case EventRecurrence.YEARLY:
                    freqField = JalaliTime.YEAR;
                    break;
                default:
            ...
            if (freqField == JalaliTime.MONTH) {
                if (useDays) {
                    ...
                    iterator.jalaliMonthDay = 1;
                    iterator.jalaliToGregorian();
                }
            }
            ...
            events: {
                while (true) {
                    ...
                    iterator.normalize(true);
//                    unsafeNormalize(iterator);

                    int iteratorYear = iterator.jalaliYear;
                    int iteratorMonth = iterator.jalaliMonth + 1;
                    int iteratorDay = iterator.jalaliMonthDay;
                    ...
                    if (SPEW) Log.i(TAG, "year=" + generated.jalaliYear);
                    ...
                    do { // month
                        ...
                        if (useDays) {
                            ...
                            if (freq == EventRecurrence.WEEKLY) {
                                ...
                                dayIndex = iterator.jalaliMonthDay - weekStartAdj;
                                lastDayToExamine = dayIndex + 6;
                            } else {
                                lastDayToExamine = generated
                                    .getJalaliActualMaximum(JalaliTime.MONTH_DAY);
                            }
...
//                                        unsafeNormalize(generated);
                    ...
                    // per month
                    int oldDay = iterator.jalaliMonthDay;
                    generated.set(iterator);  // just using generated as a temporary.
                    int n = 1;
                    while (true) {
                        int value = freqAmount * n;
                        switch (freqField) {
                            case JalaliTime.SECOND:
                                iterator.second += value;
                                break;
                            case JalaliTime.MINUTE:
                                iterator.minute += value;
                                break;
                            case JalaliTime.HOUR:
                                iterator.hour += value;
                                break;
                            case JalaliTime.MONTH_DAY:
                                iterator.jalaliMonthDay += value;
                                iterator.jalaliToGregorian();
                                break;
                            case JalaliTime.MONTH:
                                iterator.jalaliMonth += value;
                                iterator.jalaliToGregorian();
                                break;
                            case JalaliTime.YEAR:
                                iterator.jalaliYear += value;
                                iterator.jalaliToGregorian();
                                break;
                            case JalaliTime.WEEK_DAY:
                                iterator.jalaliMonthDay += value;
                                iterator.jalaliToGregorian();
                                break;
                            case JalaliTime.YEAR_DAY:
                                iterator.jalaliMonthDay += value;
                                iterator.jalaliToGregorian();
                                break;
                            default:
...
//                        unsafeNormalize(iterator);
                        iterator.normalize(true);
                        if (freqField != JalaliTime.YEAR && freqField != JalaliTime.MONTH) {
                            break;
                        }
                        if (iterator.jalaliMonthDay == oldDay) {
                            break;
                        }
                        n++;
    ...
    static int yearLength(int year) {
        return JalaliTime.isLeapYear(year) ? 366 : 365;
    }
...
//        120, 151, 180, 212, 243, 273, 304, 334 };
    ...
    private static final int[] JALALI_DAYS_IN_YEAR_PRECEDING_MONTH = { 0, 31, 62, 93,
    	124, 155, 186, 216, 246, 276, 306, 336 };
     ...
     */
    static int jalaliMonthLength(int year, int month) {
        int n = JalaliTime.JALALI_DAYS_PER_MONTH[month];
        if (n != 29) {
            return n;
        }
        return JalaliTime.isLeapYear(year) ? 30 : 29;
    }
...
//    }
    ...
    static int yearDay(int year, int month, int day) {
        int yearDay = JALALI_DAYS_IN_YEAR_PRECEDING_MONTH[month] + day - 1;
//        if (month >= 2 && JalaliTime.isLeapYear(year)) {
//            yearDay += 1;
//        }
        return yearDay;
    }
     ...
     */
    private static final long normDateTimeComparisonValue(JalaliTime normalized) {
    ...
    private static final void setTimeFromLongValue(JalaliTime date, long val) {
